...
const {
  fromCompiledFiles,
} = require('./scripts/generate-screenshots');
...
function compile() {
  ...
  function destCompiled(file, t) {
    file.path = file.path + ".js";
    return t.through(dest, ['dist/compiled']);
  }
  ...
  return src(scriptsGlob)
    ...
    .pipe(strip())
    .pipe(tap(destCompiled))
    .pipe(change(logScripts))
}

function bookmarksHtml() {

  let scriptsGlob = ['dist/compiled/*.js'];

  if (IGNORE_UNFINISHED_SCRIPTS) {
    scriptsGlob.push('!dist/compiled/_*.js');
  }
...
const build = series(compile, bookmarksHtml);
...
module.exports = {
  'dev:screenshot': series(build, fromCompiledFiles),
  new: newScript,
  build: build,
